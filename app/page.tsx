...
export default function Home() {
  ...
  async function handleScore() {
    ...
    try {
      ...
      setResult(data.result || "結果が取得できませんでした");
      await loadHistory();
    } catch (error) {
      console.error(error);
  ...
  const goodPoint = getSection(result, "良い点");
  const improvePoint = getSection(result, "改善点");
  const titleIdea = getSection(result, "タイトル案");
  const example = getSection(result, "人気キャスト風の改善例");
